  ...
  for (indicedia in 1:length(dias_com_jogo)) {
    ...
    for (indice_jogos in 1:length(jogos_html)) {
      ...
      infos_partida = (info_jogo[1] %>% 
                         str_split(' - '))[[1]]%>% 
        str_trim()
      competicao = infos_partida[length(infos_partida)-1]
      
      base_jogos_Dia = data.frame(Dia = data, Competição = competicao, Mandante = info_jogo[2],
      ...
      base_jogos_Dia = base_jogos_Dia%>% 
        mutate(Resultado = case_when(is.na(GolsMandante)~'N',
               ...
               Situação = case_when(info_jogo[6]=='ao vivo'~'Ao Vivo',
                                    ...
                                    info_jogo[6]=='pré-jogo' ~infos_partida[1],
                                    T~info_jogo[6]))
      ...
      base_jogos_Dia = base_jogos_Dia %>% 
        mutate(Minutos = ifelse(is.null(minutos),NA,minutos)) %>% 
